...
- **환경 격리 불일치**: 동일한 호스트 내에서 서로 다른 라이브러리 버전이나 포트 충돌 없이 여러 앱을 실행할 수 있는 [[본질-환경 격리 (Environment Isolation)]]를 제공합니다.
...
- [[본질-환경 격리 (Environment Isolation)]]: 프로세스 간의 간섭을 최소화하는 하위 기술.
- [[본질-최소 단위의 불일치 (Granularity Mismatch)]]: 호스트 자원과 컨테이너 요구사항 간의 차이를 조율함.
